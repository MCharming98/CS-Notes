...
网络把主机连接起来，而互连网（internet）是把多种不同的网络连接起来，因此**互连网是网络的网络**。而互联网（Internet）是全球范围的互连网。
...
-   **应用层Application Layer**  ：为特定应用程序（如浏览器）提供数据传输服务，例如 HTTP（port80）、DNS、smtp（邮件） 等协议。数据单位为报文（message）。

-   **传输层Transport Layer**  ：为进程提供通用数据传输服务。由于应用层协议很多，定义通用的传输层协议就可以支持不断增多的应用层协议。
                                运输层包括两种协议：传输控制协议 TCP，提供面向连接、可靠的数据传输服务，数据单位为报文段（message chunk）；
                                用户数据报协议 UDP，提供无连接、尽最大努力的数据传输服务，数据单位为用户数据报。TCP 主要提供完整性服务，UDP 主要提供及时性服务。

-   **网络层Network Layer**  ：为主机提供数据传输服务。而传输层协议是为主机中的进程提供数据传输服务。网络层把传输层传递下来的报文段或者用户数据报封装成分组。
                              包含IP(Internet Protocol)

-   **数据链路层Data Link Layer**  ：网络层针对的还是主机之间的数据传输服务，而主机之间可以有很多链路，链路层协议就是为同一链路的主机提供数据传输服务。数据链路层把网络层传下来的分组封装成帧。
                                 包含以太网(Ethernet) -> mac address, 交换机(Switch)

-   **物理层Physical Layer**  ：考虑的是怎样在传输媒体上传输数据比特流，而不是指具体的传输媒体。物理层的作用是尽可能屏蔽传输媒体和通信手段的差异，使数据链路层感觉不到这些差异。
                                 包含网线，网卡
...
## 3. TCP(Transmission Control Protocal)/IP(Internet Protocol) 

它只有四层，相当于五层协议中数据链路层和物理层合并为网络接口层(Link Layer)。

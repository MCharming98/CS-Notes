...
进程控制块 (Process Control Block, PCB) 描述进程的**基本信息和运行状态**(Metadata)，所谓的创建进程和撤销进程，都是指对 PCB 的操作。
...
<div align="center"> <img src="https://i.stack.imgur.com/xAPl5.jpg"/> </div><br>

## 3. 区别

Ⅰ 拥有资源

进程是资源分配的基本单位，但是**线程不拥有资源**，线程可以**访问隶属进程的资源**。

Ⅱ 调度

线程是独立调度的基本单位，在同一进程中，**线程的切换不会引起进程切换**，从一个**进程中的线程切换到另一个进程中的线程时，会引起进程切换**。
...
线程间可以通过直接读写同一进程中的数据进行通信，但是**进程通信需要借助 IPC**。
...
- 同步 Synchronization：多个进程因为合作产生的直接制约关系，使得进程有一定的先后执行关系。
- 互斥 Mutual Exclusive：多个进程在同一时刻只有一个进程能进入临界区。
